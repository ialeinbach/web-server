...
function config_ssl() {
	...
	if [[ -n "$cert" ]]; then
		echo "ssl_certificate /$SSL_DIR/$(basename $cert);" >> $SSL_CONF
	fi
	if [[ -n "$key"  ]]; then
		echo "ssl_certificate_key /$SSL_DIR/$(basename $key);" >> $SSL_CONF
	fi
	if [[ -n "$pass" ]]; then
		echo "ssl_password_file /$SSL_DIR/$(basename $pass);" >> $SSL_CONF
	fi
}
